import { ModelTypes, WorkCredential, WorkCredentials } from "@/interfaces";
import { PublicRecord, usePublicRecord } from "@self.id/react";
...
export function useWCRecords(
  did: string
): PublicRecord<WorkCredentials | null> {
  return usePublicRecord<ModelTypes, "workCredentials">("workCredentials", did);
}

export function useWCRecord(id?: string): TileDoc<WorkCredential> {
